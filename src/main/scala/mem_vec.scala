...
class MemVecSample (addrBits : Int) extends Module {
  val io = new Bundle {
  ...
  // Mem of Vec does not work well.
  val value = Mem (Vec(UInt (width = 32),numOfMem),numOfMem);
...
object MemVecSample {
  ...
  def main (args : Array[String]) : Unit = {
    chiselMainTest (args,() => Module (new MemVecSample (addrBits))) {
      c => new MemVecSampleTests (c)
    }
  }

  class MemVecSampleTests (c: MemVecSample) extends Tester (c) {
