...
class MemSample (addrBits : Int) extends Module {
  val io = new Bundle {
  ...
  val value = Mem (UInt (width = 32),numOfMem);
...
object MemSample {
  ...
  def main (args : Array[String]) : Unit = {
    chiselMainTest (args,() => Module (new MemSample (addrBits))) {
      c => new MemSampleTests (c)
    }
  }

  class MemSampleTests (c: MemSample) extends Tester (c) {
